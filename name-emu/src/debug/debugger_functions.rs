...
// pub type DebugFn = fn(&Vec<LineInfo>, &mut Memory, &mut Processor, &mut DebuggerState, &Vec<String>) -> Result<(), String>;
        ...
        Breakpoint {
            ...
            address: {
                match lineinfo.iter().find(|&line| line.line_number == line_num) {
                    ...
                    None => {
                        panic!("Breakpoint not found in memory."); // there is no reason this should be happening,
                    }
                }
            },
        }
...
// This implementation contains methods that only affect the debugger state and do not (necessarily) modify the program state.
impl DebuggerState {
    ...
    pub fn add_breakpoint(
        ...
        let line_num = match db_args[1].parse() {
            Ok(thing) => thing,
            Err(_) => return Err(format!("b takes a 32-bit unsigned int as input")),
        };
    ...
    pub fn remove_breakpoint(&mut self, db_args: &Vec<String>) -> Result<(), String> {
        ...
        let bp_num = match db_args[1].parse() {
            Ok(thing) => thing,
            Err(_) => return Err(format!("del takes a 16-bit unsigned int as input")),
        };
            ...
            .position(|brpt| brpt.bp_num == bp_num)
        {
            ...
            for bp in &mut self.breakpoints {
                // if we don't do this for loop, then we'll have n empty numbers
                bp.bp_num -= 1;
            }
            Ok(())
        } else {
    ...
    pub fn list_lines(&mut self, lineinfo: &Vec<LineInfo>, mut lnum: usize) {
        if lnum == 0 {
            lnum = self.global_list_loc;  // trollface
        }
        ...
        // to the line number that was just fed in.
        // in a hypothetical future, we can add a flag to make this an option i guess
...
pub fn print_register(cpu: &mut Processor, db_args: &Vec<String>) -> Result<(), String> {
    if db_args.len() < 2 {
        return Err(format!(
            "p expects a non-zero number of arguments, received {}",
            db_args.len() - 1
        ));
    }
...
pub fn help_menu(args: Vec<String>) {
    if args.len() == 1 {
        ...
        println!("q, exit, quit - Exit (quit) debugger.");
    } else if args.len() == 2 {
        match &args[1] as &str {
            "help" => {
