use crate::decode::{unpack_i_type, unpack_j_type, unpack_r_type};
...
pub fn jr(
    ...
    if cpu.general_purpose_registers[rs] >= memory.text_end
        || cpu.general_purpose_registers[rs] < memory.text_start
    {
        return Err(format!(
            "Attempted to jump to unowned address 0x{:x}",
            cpu.general_purpose_registers[rs]
        ));
    }
...
pub fn add(
    ...
    cpu.general_purpose_registers[rd] =
        cpu.general_purpose_registers[rs] + cpu.general_purpose_registers[rt];
...
pub fn addu(
    ...
    cpu.general_purpose_registers[rd] = cpu.general_purpose_registers[rs]
        .overflowing_add(cpu.general_purpose_registers[rt])
        .0;
    Ok(ExecutionStatus::Continue)
}
...
pub fn sub(
    ...
    let temp: (u32, bool) =
        cpu.general_purpose_registers[rs].overflowing_sub(cpu.general_purpose_registers[rt]);
...
pub fn addi(
    ...
    let (rs, rt, imm) = unpack_i_type(instruction);
    cpu.general_purpose_registers[rt] =
        (cpu.general_purpose_registers[rs] as i32 + (imm as i16 as i32)) as u32;
    Ok(ExecutionStatus::Continue)
}
...
pub fn lb(
    ...
    cpu.general_purpose_registers[AS_TEMP] =
        (cpu.general_purpose_registers[rs] as i32 + imm as i32) as u32;

    if cpu.general_purpose_registers[AS_TEMP] >= memory.data_end
        || cpu.general_purpose_registers[AS_TEMP] < memory.data_start
    {
        return Err(format!(
        ...
        cpu.general_purpose_registers[rt] = memory.data
            [(cpu.general_purpose_registers[AS_TEMP] - memory.data_start) as usize]
            as u32;
    }
...
pub fn sw(
    ...
    memory.data.splice(
        start_idx..end_idx,
        cpu.general_purpose_registers[rt].to_be_bytes(),
    );
