    ...
    while current_section < 6 {
        match current_section {
            1 => {
                ...
                let data_offsets: Vec<u32> = offsets.iter().map(|elf_offsets| elf_offsets[current_section]).collect();
                let pads: Vec<usize> = elfs.iter().enumerate().map(|(idx, elf)| if idx != elfs.len()-1 {(data_offsets[idx+1] - elf.section_header_table[current_section].sh_size) as usize} else {0}).collect();
                let padded_datas: Vec<u8> = elfs.iter().enumerate().map(|(idx, elf)| vec![elf.sections[current_section].clone(), vec![0u8; pads[idx]]]).flatten().flatten().collect();
                return_vector.push(padded_datas);
            },
            _ => {
                // Anything else -> one-liner.
                return_vector.push(elfs.iter().map(|elf| elf.sections[current_section].clone()).flatten().collect());
            }
        }
...
fn validate_consolidation() {
    let elf1: Elf = name_core::elf_utils::create_new_elf(vec![vec![0u8; 37]; 6], name_core::elf_def::ElfType::Relocatable);
    let elf2: Elf = elf1.clone();

    let elfs: Vec<Elf> = vec![elf1, elf2];

    let offsets: Vec<Vec<u32>> = crate::offsets::calculate_offsets(&elfs);
