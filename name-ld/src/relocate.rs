...
pub fn relocate(sections: Vec<Vec<u8>>, offsets: Vec<Vec<u32>>) -> Result<Elf, String> {
    ...
    let relocated_symtab: Elf = relocate_symtab_links(new_elf, &offsets);
    ...
    let checked_symbols: Elf = check_duplicate_symbols(relocated_symtab, &offsets)?;

    // Now, each entry in .rel needs to be reconciled.
    // This is a complex process, so it's also been extracted to a function.
    let relocated = relocate_text_entries(checked_symbols, &offsets);
    ...
    let new_sections: Vec<Vec<u8>> = relocated.sections.iter().enumerate().filter_map(|(idx, section)| match idx {
        0 | 2 => None,
