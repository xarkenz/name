...
pub fn relocate(elfs: &Vec<Elf>, _external_symbols: &Vec<Elf32Sym>) -> Result<Elf, String> {
    // Consolidate sections
    // .data
    let mut consolidated_data: Vec<u8> = vec![];
    for elf in elfs {
        consolidated_data.extend(elf.sections[0].clone());
    ...
    // .text
    let consolidated_text: Vec<u8> = elfs
        .iter()
        .map(|elf| elf.sections[1].clone())
        .flatten()
        .collect();
    // skip .rel
    // .symtab
    let consolidated_symtab: Vec<u8> = elfs
        .iter()
        .map(|elf| elf.sections[3].clone())
        .flatten()
        .collect();
    // .strtab
    let consolidated_strtab: Vec<u8> = elfs
        .iter()
        .map(|elf| elf.sections[4].clone())
        .flatten()
        .collect();
    // .line
    let consolidated_line: Vec<u8> = elfs
        .iter()
        .map(|elf| elf.sections[5].clone())
        .flatten()
        .collect();
