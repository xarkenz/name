...
pub fn relocatable_conformity_check(et_rel: &Elf) -> Result<(), String> {
    ...
    if fh.e_type != ET_REL {
        return Err(format!("Linker expected a relocatable object file ({}), found e_type field {}.", E_TYPE_DEFAULT, fh.e_type));
    }

    if fh.e_machine != E_MACHINE_DEFAULT {
        return Err(format!("NAME only supports big-endian MIPS ({}), received {}.", E_MACHINE_DEFAULT, fh.e_machine));
    }

    if fh.e_phoff != E_PHOFF_DEFAULT {
        return Err(format!("Non-compliant file header size detected. Ensure 32-bit format."));
    }

    if fh.e_flags != E_FLAGS_DEFAULT {
        return Err(format!("Linker expected flags {}, received {}.", E_FLAGS_DEFAULT, fh.e_flags));
    }
    ...
    if fh.e_phentsize != E_PHENTSIZE_DEFAULT {
        return Err(format!("Non-compliant program header entry size detected. Ensure 32-bit format."));
    }
    ...
    if fh.e_shentsize != E_SHENTSIZE_DEFAULT {
        return Err(format!("Non-compliant section header entry size detected. Ensure 32-bit format."));
    }
...
fn check_e_ident(e_ident: &[u8]) -> Result<(), String> {
    if e_ident[0..4] != E_IDENT_DEFAULT[0..4] {
        return Err(format!("Magic bytes did not match for ELF format (EI_MAG field expected {:x}).", u32::from_be_bytes(E_IDENT_DEFAULT[0..4].try_into().unwrap())));
    }

    if e_ident[5] != E_IDENT_DEFAULT[5] {
        return Err(format!("NAME currently only supports 32-bit binaries (EI_CLASS field expected {}).", EI_CLASS));
    }

    if e_ident[6] != E_IDENT_DEFAULT[6] {
        return Err(format!("NAME only supports big-endian data (EI_DATA field expected {}).", EI_DATA));
    }
